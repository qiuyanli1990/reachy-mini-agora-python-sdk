...
class LocalStream:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the stream with a conversational handler and pipelines.
        """
        ...
        self._settings_app: Optional[object] = settings_app
        self._instance_path: Optional[str] = instance_path

    def launch(self) -> None:
        """Start the recorder/player and run the async processing loops.

        Configuration is read directly from environment variables and
        ``agent_config.json``. No runtime settings UI gating is applied.
        """
        ...
        async def runner() -> None:
            self._tasks = [
                asyncio.create_task(self.handler.start_up(), name="agora-handler"),
    ...
    def close(self) -> None:
        ...
        - Cancels all pending async tasks (agora-handler, record-loop, play-loop)
        """
    ...
    async def play_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if isinstance(handler_output, AdditionalOutputs):
                ...
                # Apply configurable playback attenuation/gain.
                playback_volume = float(
                    os.getenv("PLAYBACK_VOLUME", "1.0")
                )
                playback_volume = min(max(playback_volume, 0.0), 1.0)
                audio_frame = np.clip(
                    audio_frame * playback_volume,
                    -1.0,
                    1.0,
