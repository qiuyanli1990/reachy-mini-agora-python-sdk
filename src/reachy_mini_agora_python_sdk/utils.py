...
from reachy_mini_agora_python_sdk.camera_worker import CameraWorker
...
def parse_args() -> Tuple[argparse.Namespace, list]:  # type: ignore
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use local vision model for camera tool analysis",
    )
...
def handle_vision_stuff(args: argparse.Namespace, current_robot: ReachyMini) -> Tuple[CameraWorker | None, Any, Any]:
    ...
    By default, camera frames are only used by tools that request them.
    If --local-vision flag is used, a local vision model processes images periodically.
    """
    camera_worker = None
    head_tracker = None
    vision_manager = None
    ...
    if not args.no_camera:
        ...
        if args.head_tracker is not None:
            if args.head_tracker == "yolo":
                from reachy_mini_agora_python_sdk.vision.yolo_head_tracker import HeadTracker
        ...
        if args.local_vision:
            try:
                from reachy_mini_agora_python_sdk.vision.processors import initialize_vision_manager
            ...
            logging.getLogger(__name__).info(
                "Using default camera tool mode (no local vision model). Use --local-vision for local processing.",
            )
